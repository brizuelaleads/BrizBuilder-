...
export function ReportsView({ leads, clients, metaAdInsights }: { leads: CrmLead[]; clients: CrmClient[]; metaAdInsights: CrmMetaAdInsight[] }) {
  ...
  // Spend is what the ad accounts actually reported, not the budget somebody
  // configured: a budget is an intention, and dividing revenue by it reports a
  // return on money that may never have been spent.
  const spend = metaAdInsights.reduce((sum, insight) => sum + insight.spendCents, 0);
  const budget = clients.reduce((sum, client) => sum + client.monthlyAdBudgetCents, 0);
  const revenue = leads.reduce((sum, lead) => sum + (lead.status === "WON" ? lead.finalRevenueCents : 0), 0);
  // And only revenue from leads an ad actually delivered counts against it, the
  // rule the Ads tab and the dashboard both follow (lib/meta-ads-report.ts).
